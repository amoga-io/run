...
func TestPm2ScriptLocation(t *testing.T) {
	tests := []struct {
		name         string
...
exit 0`
		...
		},
		{
			...
			expectScript: false,
			expectError:  true,
			expectedErr:  "PM2 installation script not found",
		},
		{
...
exit 0`
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if _, err := os.Stat(scriptPath); os.IsNotExist(err) {
				if tt.expectError && !strings.Contains(tt.expectedErr, "PM2 installation script not found") {
...
func TestPm2CommandProperties(t *testing.T) {
	t.Run("command has correct properties", func(t *testing.T) {
		...
		if !strings.Contains(Cmd.Long, "Install PM2 on your system") {
			t.Errorf("Expected Long to contain 'Install PM2 on your system', got '%s'", Cmd.Long)
		}
		if Cmd.Run == nil {
...
func TestPm2UtilsGetScriptPath(t *testing.T) {
	t.Run("GetScriptPath returns correct path format", func(t *testing.T) {
		scriptPath := utils.GetScriptPath("pm2.sh")

		// Check that the path ends with the expected structure
		if !strings.HasSuffix(scriptPath, "/.devkit/scripts/pm2.sh") {
			t.Errorf("Expected script path to end with '/.devkit/scripts/pm2.sh', got '%s'", scriptPath)
		}
...
func TestPm2ScriptPermissions(t *testing.T) {
	t.Run("script can be made executable", func(t *testing.T) {
		// Create a temporary script
		tempDir := t.TempDir()
		scriptPath := filepath.Join(tempDir, "test-pm2.sh")
...
exit 0`
		...
		// Make it executable (same as done in pm2.go)
