...
func TestPostgresScriptLocation(t *testing.T) {
	tests := []struct {
		name         string
	...
	}{
		{
			...
			setupFunc: func(t *testing.T) (string, func()) {
				...
				scriptPath := filepath.Join(scriptDir, "postgres17.sh")
				scriptContent := `#!/bin/bash
# Mock postgres installation script
...
exit 0`
		...
		},
		{
			...
			expectScript: false,
			expectError:  true,
			expectedErr:  "postgres installation script not found",
		},
		{
...
exit 0`
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if _, err := os.Stat(scriptPath); os.IsNotExist(err) {
				if tt.expectError && !strings.Contains(tt.expectedErr, "postgres installation script not found") {
...
func TestPostgresCommandProperties(t *testing.T) {
	t.Run("command has correct properties", func(t *testing.T) {
		if Cmd.Use != "postgres" {
			t.Errorf("Expected Use to be 'postgres', got '%s'", Cmd.Use)
		}
		if Cmd.Short != "Install postgres" {
			t.Errorf("Expected Short to be 'Install postgres', got '%s'", Cmd.Short)
		}
		if !strings.Contains(Cmd.Long, "Install postgres on your system") {
			t.Errorf("Expected Long to contain 'Install postgres on your system', got '%s'", Cmd.Long)
		}
		if Cmd.Run == nil {
...
func TestPostgresUtilsGetScriptPath(t *testing.T) {
	t.Run("GetScriptPath returns correct path format", func(t *testing.T) {
		scriptPath := utils.GetScriptPath("postgres17.sh")

		// Check that the path ends with the expected structure
		if !strings.HasSuffix(scriptPath, "/.devkit/scripts/postgres17.sh") {
			t.Errorf("Expected script path to end with '/.devkit/scripts/postgres17.sh', got '%s'", scriptPath)
		}
...
func TestPostgresScriptPermissions(t *testing.T) {
	t.Run("script can be made executable", func(t *testing.T) {
		// Create a temporary script
		tempDir := t.TempDir()
		scriptPath := filepath.Join(tempDir, "test-postgres17.sh")
...
exit 0`
		...
		// Make it executable (same as done in postgres.go)
