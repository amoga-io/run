...
func TestNginxScriptLocation(t *testing.T) {
	tests := []struct {
		name         string
...
# Mock nginx installation script
...
exit 0`
		...
		},
		{
			...
			expectScript: false,
			expectError:  true,
			expectedErr:  "Nginx installation script not found",
		},
		{
...
exit 0`
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if _, err := os.Stat(scriptPath); os.IsNotExist(err) {
				if tt.expectError && !strings.Contains(tt.expectedErr, "Nginx installation script not found") {
...
func TestNginxCommandProperties(t *testing.T) {
	t.Run("command has correct properties", func(t *testing.T) {
		...
		if !strings.Contains(Cmd.Long, "Install Nginx on your system") {
			t.Errorf("Expected Long to contain 'Install Nginx on your system', got '%s'", Cmd.Long)
		}
		if Cmd.Run == nil {
...
func TestNginxUtilsGetScriptPath(t *testing.T) {
	t.Run("GetScriptPath returns correct path format", func(t *testing.T) {
		scriptPath := utils.GetScriptPath("nginx.sh")

		// Check that the path ends with the expected structure
		if !strings.HasSuffix(scriptPath, "/.devkit/scripts/nginx.sh") {
			t.Errorf("Expected script path to end with '/.devkit/scripts/nginx.sh', got '%s'", scriptPath)
		}
...
func TestNginxScriptPermissions(t *testing.T) {
	t.Run("script can be made executable", func(t *testing.T) {
		// Create a temporary script
		tempDir := t.TempDir()
		scriptPath := filepath.Join(tempDir, "test-nginx.sh")
...
exit 0`
		...
		// Make it executable (same as done in nginx.go)
