...
// SupportsVersion checks if a package supports version selection
func SupportsVersion(packageName string) bool {
	pkg, exists := GetPackage(packageName)
	return exists && pkg.VersionSupport
}

// ValidateVersion validates if a version is supported for a package
...
// GetDefaultVersion returns the default version for a package
...
// GetSupportedVersions returns the list of supported versions for a package
